...
static unsigned int delayRunApplicationEnabled = 1;
static unsigned long delayRunApplication = 0xFFFFF;
...
UINT16 CalculateCrc_WithoutEeprom(UINT8 *data, UINT32 len)
{
	...
	while(len--)
	{
		if (((unsigned int)data >= EEPROM_FLASH_BASE_ADDRESS) && ((unsigned int)data <= EEPROM_FLASH_END_ADDRESS))
		{
			i = (crc >> 12) ^ (0xFF >> 4);
			crc = crc_table[i & 0x0F] ^ (crc << 4);
			i = (crc >> 12) ^ (0xFF >> 0);
