...
LorawanError_t LORAWAN_Join(ActivationType_t activationTypeNew)
{
    ...
    if (OTAA == activationTypeNew)
    {
        ...
        else
        {
            ...
            if (result == OK)
            {
                if (RADIO_Transmit(macBuffer, bufferIndex) == ERR_NONE)
                {
    ...
    else
    {
        ...
        else
        {
            ...
            SwTimerSetTimeout(loRa.abpJoinTimerId, ABP_TIMEOUT_MS);
...
LorawanError_t LORAWAN_Send (TransmissionType_t confirmed, uint8_t port,  void *buffer, uint8_t bufferLength)
 {
    ...
    else
    {
       ...
       if (RADIO_Transmit (&macBuffer[16], loRa.lastPacketLength) == ERR_NONE)
       {
...
void LORAWAN_LinkCheckConfigure (uint16_t period)
{
    ...
    else
    {
        ...
        {
           SwTimerSetTimeout(loRa.linkCheckTimerId, loRa.periodForLinkCheck);
...
void LORAWAN_ReceiveWindow1Callback (uint8_t param)
{
    ...
    if(loRa.macStatus.macPause == DISABLED)
    {
		...
		sx1276_rx_started = getGlobalTimeUs_Lora();
        ConfigureRadioRx(loRa.receiveWindow1Parameters.dataRate, freq);
		#ifdef SX1276_WIDER_RX_WINDOW
...
void LORAWAN_ReceiveWindow2Callback(uint8_t param)
{
    ...
    if (loRa.macStatus.macPause == DISABLED)
    {
        if((RADIO_GetStatus() & RADIO_FLAG_RECEIVING) == 0)
        {
            ...
            if (RADIO_ReceiveStart(rxWindowSize[loRa.receiveWindow2Parameters.dataRate]) != ERR_NONE)
            {
...
void UnconfirmedTransmissionCallback (uint8_t param)
{
    ...
    if ( (SelectChannelForTransmission (1) == OK) && (RADIO_Transmit (&macBuffer[16], loRa.lastPacketLength) == ERR_NONE) )
    {
...
void UpdateRetransmissionAckTimeoutState (void)
{
    ...
    SwTimerSetTimeout(loRa.ackTimeoutTimerId, loRa.protocolParameters.ackTimeout);
...
LorawanError_t LORAWAN_RxDone (uint8_t *buffer, uint8_t bufferLength)
{
    ...
    if (loRa.macStatus.macPause == DISABLED)
    {
		sx1276_rx_completed = getGlobalTimeUs_Lora();

		sx1276_rx_delay = sx1276_rx_completed - sx1276_tx_completed;
...
{
	#ifdef SX1276_DIO0_PORT
		static volatile uint8 SX1276_DIO0_PORT_prev = 0;
	#endif
	#ifdef SX1276_DIO1_PORT
		static volatile uint8 SX1276_DIO1_PORT_prev = 0;
	#endif
	#ifdef SX1276_DIO2_PORT
		static volatile uint8 SX1276_DIO2_PORT_prev = 0;
	#endif
	#ifdef SX1276_DIO3_PORT
		static volatile uint8 SX1276_DIO3_PORT_prev = 0;
	#endif
	#ifdef SX1276_DIO4_PORT
		static volatile uint8 SX1276_DIO4_PORT_prev = 0;
	#endif
	#ifdef SX1276_DIO5_PORT
		static volatile uint8 SX1276_DIO5_PORT_prev = 0;
	#endif
	volatile uint8 current_val = 0;
	volatile uint8 prev_val = 0;
	SwTimersExecute();
		...
		prev_val = SX1276_DIO0_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO0();
		}
		if (current_val) {
			SX1276_DIO0_PORT_prev = 1;
		} else {
			SX1276_DIO0_PORT_prev = 0;
		}
	#endif
	#ifdef SX1276_DIO1_PORT
		if (SX1276_DIO1_PORT) {
		...
		prev_val = SX1276_DIO1_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO1();
		}
		if (current_val) {
			SX1276_DIO1_PORT_prev = 1;
		} else {
			SX1276_DIO1_PORT_prev = 0;
		}
	#endif
	#ifdef SX1276_DIO2_PORT
		if (SX1276_DIO2_PORT) {
		...
		prev_val = SX1276_DIO2_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO2();
		}
		if (current_val) {
			SX1276_DIO2_PORT_prev = 1;
		} else {
			SX1276_DIO2_PORT_prev = 0;
		}
	#endif
	#ifdef SX1276_DIO3_PORT
		if (SX1276_DIO3_PORT) {
		...
		prev_val = SX1276_DIO3_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO3();
		}
		if (current_val) {
			SX1276_DIO3_PORT_prev = 1;
		} else {
			SX1276_DIO3_PORT_prev = 0;
		}
	#endif
	#ifdef SX1276_DIO4_PORT
		if (SX1276_DIO4_PORT) {
		...
		prev_val = SX1276_DIO4_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO4();
		}
		if (current_val) {
			SX1276_DIO4_PORT_prev = 1;
		} else {
			SX1276_DIO4_PORT_prev = 0;
		}
	#endif
	#ifdef SX1276_DIO5_PORT
		if (SX1276_DIO5_PORT) {
		...
		prev_val = SX1276_DIO5_PORT_prev;

		if ((prev_val == 0) && (current_val != 0)) {
			RADIO_DIO5();
		}
		if (current_val) {
			SX1276_DIO5_PORT_prev = 1;
		} else {
			SX1276_DIO5_PORT_prev = 0;
		}
	#endif
}
