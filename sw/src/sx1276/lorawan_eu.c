...
static uint16_t sx1276_read_uint16_unaligend(uint8_t *ptr);
static uint32_t sx1276_read_uint32_unaligend(uint8_t *ptr);
...
void LORAWAN_Reset (IsmBand_t ismBandNew)
{
    ...
    if(ISM_EU868 == ismBandNew)
    {
        ...
        SwTimerSetTimeout(loRa.otaaJoinTimerId, OTAA_MINUTECOUNT_MS);
...
void LORAWAN_TxDone(uint16_t timeOnAir)
{
    if (loRa.macStatus.macPause == DISABLED)
    {
       ...
       {           
            SwTimerSetTimeout(loRa.joinAccept1TimerId, loRa.protocolParameters.joinAcceptDelay1 + rxWindowOffset[loRa.receiveWindow1Parameters.dataRate]);
            SwTimerSetTimeout(loRa.joinAccept2TimerId, loRa.protocolParameters.joinAcceptDelay2 + rxWindowOffset[loRa.receiveWindow2Parameters.dataRate]);
       ...
       {
			sx1276_tx_completed = getGlobalTimeUs_Lora();
			#ifdef SX1276_WIDER_RX_WINDOW
				SwTimerSetTimeout(loRa.receiveWindow1TimerId, 1);
				//SwTimerSetTimeout(loRa.receiveWindow1TimerId, (loRa.protocolParameters.receiveDelay1 + rxWindowOffset[loRa.receiveWindow1Parameters.dataRate]) / 2);
				SwTimerSetTimeout(loRa.receiveWindow2TimerId, 60000);
			#else
				SwTimerSetTimeout(loRa.receiveWindow1TimerId, loRa.protocolParameters.receiveDelay1 + rxWindowOffset[loRa.receiveWindow1Parameters.dataRate]);
				SwTimerSetTimeout(loRa.receiveWindow2TimerId, loRa.protocolParameters.receiveDelay2 + rxWindowOffset[loRa.receiveWindow2Parameters.dataRate]);
			#endif
       ...
       if (found == true)
       {
           ...
           SwTimerSetTimeout (loRa.dutyCycleTimerId, minim);
...
void LORAWAN_RxTimeout(void)
{
    ...
    if (loRa.macStatus.macPause == 0)
    {
        ...
        {
				sx1276_rx_timeout = getGlobalTimeUs_Lora();
                    ...
                    if (loRa.counterRepetitionsUnconfirmedUplink <= loRa.maxRepetitionsUnconfirmedUplink)
                    {
                        ...
                        else
                        {
                            ...
                            for (i = 0; i <= loRa.maxChannels; i ++)
                            {
                             ...
                             SwTimerSetTimeout (loRa.unconfirmedRetransmisionTimerId, minim + 50);
...
uint8_t* ExecuteLinkAdr (uint8_t *ptr, uint8_t *commandsLen)
{
    ...
    while(ptr < commandsLen)
    {
        ...
        else if (*(ptr) == LINK_ADR_CID)
        {
            ...
            ptr++;
            channelMask = sx1276_read_uint16_unaligend(ptr);
...
void StartReTxTimer(void)
{
    ...
    loRa.macStatus.macState = RETRANSMISSION_DELAY;
    SwTimerSetTimeout (loRa.automaticReplyTimerId, minim );
