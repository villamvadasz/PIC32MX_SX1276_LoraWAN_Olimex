...
uint32 RADIO_WriteMode_mode_change_failed = 0;
...
static void RADIO_WriteMode(RadioMode_t newMode, RadioModulation_t newModulation, uint8_t blocking)
{
    ...
    if (newMode != currentMode)
    {
        ...
        if (1 == blocking)
        {
            ...
            {
				#ifdef RADIO_DRIVER_SX1276_MODECHANGE_USE_INTERRUPT
					{
						//RX ready worst case 2.33ms+5%
						unsigned int failsafe_loop_exiter = 5; //[ms]
						while (failsafe_loop_exiter) {

						    currentMode = RADIO_RegisterRead(REG_OPMODE);
							currentMode &= 0x07;
							if (currentMode == newMode) {
								break;
							}

							//{//Does not work since interrupts are masked
						    //	uint8_t irqFlags = 0;
						    //	irqFlags = RADIO_RegisterRead(REG_FSK_IRQFLAGS1);
						    //	if (1 == (irqFlags & (1<<SHIFT7))) {
							//		break;
							//	}
							//}
			                SystemBlockingWaitMs(1);
							failsafe_loop_exiter--;
						}
						if (failsafe_loop_exiter == 0) {
							RADIO_WriteMode_mode_change_failed ++;
...
RadioError_t RADIO_Transmit(uint8_t *buffer, uint8_t bufferLen)
{
    ...
    SwTimerSetTimeout(RadioConfiguration.timeOnAirTimerId, TIME_ON_AIR_LOAD_VALUE);
    SwTimerStart(RadioConfiguration.timeOnAirTimerId);

    if (0 != RadioConfiguration.watchdogTimerTimeout)
    {
        SwTimerSetTimeout(RadioConfiguration.watchdogTimerId, RadioConfiguration.watchdogTimerTimeout);
...
RadioError_t RADIO_ReceiveStart(uint16_t rxWindowSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            SwTimerSetTimeout(RadioConfiguration.fskRxWindowTimerId, rxWindowSize);
